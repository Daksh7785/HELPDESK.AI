    ...
    const AdminDashboard = () => {
        ...
        const metrics = useMemo(() => {
            ...
            const active = filteredTickets.filter(
                t =>
                !t.status?.toLowerCase()?.includes('resolv') &&
                !t.status?.toLowerCase()?.includes('closed')
            ).length;

            const autoResolved = filteredTickets.filter(
                t => t.status?.toLowerCase()?.includes('auto')
            ).length;

            const humanEscalated = filteredTickets.filter(
                t =>
        ...
        return (
            <div style={{ background: '#f8faf9', minHeight: '100vh', paddingBottom: '40px' }} className="space-y-10 -m-6 p-6 md:-m-10 md:p-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <button className="text-left group focus:outline-none">
    <StatCard
