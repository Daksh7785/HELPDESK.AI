        ...
        <header className="w-full bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 sticky top-0 z-50 transition-colors duration-200">
            <div className="max-w-[1100px] mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
                ...
                <nav className="hidden md:flex items-center gap-8">
                    <Link className="text-sm font-semibold text-gray-900 dark:text-slate-100 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors" to="/dashboard">Dashboard</Link>
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                    ...
                    <div className="hidden md:block">
                        <Avatar
                            ...
                            className="size-9 border border-gray-200 dark:border-slate-800 cursor-pointer hover:ring-2 hover:ring-emerald-500 hover:ring-offset-2 transition-all"
                        >
                            <AvatarImage src={profile?.profile_picture} />
                            <AvatarFallback className="bg-gray-100 dark:bg-slate-800 font-bold text-gray-600 dark:text-slate-300 text-xs">{initials}</AvatarFallback>
                        </Avatar>
                    </div>
                    <button
                        onClick={() => setIsMenuOpen(!isMenuOpen)}
                        className="md:hidden p-2 text-gray-600 dark:text-slate-400 focus:outline-none"
                    >
            ...
            {/* Mobile Menu Overlay */}
            {isMenuOpen && (
                <div className="md:hidden bg-white dark:bg-slate-900 border-t border-gray-100 dark:border-slate-800 absolute w-full shadow-2xl z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="px-6 py-8 space-y-6">
                        <div className="flex items-center gap-4 border-b border-gray-50 dark:border-slate-800 pb-6">
                            <Avatar className="size-12 border border-gray-100 dark:border-slate-800">
                                <AvatarImage src={profile?.profile_picture} />
                                <AvatarFallback className="bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 font-black">{initials}</AvatarFallback>
                            </Avatar>
                            <div>
