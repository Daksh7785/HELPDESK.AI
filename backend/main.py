...
# ── Ensemble + Monitoring (Issue #2805) ──────────────────────────────────────
try:
    from backend.services.ensemble_classifier import ensemble_classifier
    from backend.services.model_monitoring import model_monitor
    from backend.models.ensemble_prediction import EnsemblePrediction, ABTestRecord
    _ENSEMBLE_AVAILABLE = True
except Exception as _ensemble_import_err:
    print(f"[WARNING] Ensemble classifier not loaded: {_ensemble_import_err}")
...
async def analyze_only(request_body: TicketRequest):
    ...
    # --- Classification (Ensemble + Single-model fallback) ---
    ensemble_meta = {}  # Extra ensemble metadata for the response
    try:
        if _ENSEMBLE_AVAILABLE and ensemble_classifier:
            # ── Ensemble pipeline (Issue #2805) ──────────────────────────────
            import hashlib as _hashlib
            _text_hash = _hashlib.sha256(text.encode()).hexdigest()[:16]
            ensemble_result = ensemble_classifier.predict_with_metadata(text)

            cat = ensemble_result["category"]
            sub = ensemble_result["subcategory"]
            conf = ensemble_result["confidence"]

            from backend.services.classifier_service import TEAM_MAP, AUTO_RESOLVE_SUBS, PRIORITY_MAP
            assigned_team = TEAM_MAP.get(cat, "General Support")
            auto_resolve = sub in AUTO_RESOLVE_SUBS
            priority = PRIORITY_MAP.get(sub, "Medium")
            ...
            # Capture ensemble-specific metadata for response enrichment
            ensemble_meta = {
                "ensemble_entropy": ensemble_result.get("entropy", 0.0),
                "ensemble_agreement": ensemble_result.get("agreement", 0.0),
                "ensemble_routing_action": ensemble_result.get("routing_action", "auto_route"),
                "model_votes": ensemble_result.get("model_votes", {}),
                "individual_confidences": ensemble_result.get("individual_confidences", {}),
            }

            # Override needs_review based on ensemble routing action
            if ensemble_result.get("routing_action") in ("human_review", "escalate"):
                classification["needs_review"] = True

            # Log prediction for drift monitoring
            if model_monitor:
                model_monitor.log_prediction(ensemble_result)

            # Build A/B test record (compare with V3 single model)
            try:
                v3_res = classifier_v3.predict(text)
                if "error" not in v3_res and model_monitor:
                    ab_rec = ABTestRecord(
                        ticket_id=str(uuid.uuid4()),
                        input_text_hash=_text_hash,
                        single_model_prediction=f"{v3_res.get('Category', {}).get('prediction', 'Unknown')} | {v3_res.get('Subcategory', {}).get('prediction', 'Unknown')}",
                        single_model_confidence=float(v3_res.get("Category", {}).get("confidence", 0.0)),
                    ...
                    model_monitor.log_ab_record(ab_rec.to_dict())
            except Exception:
                pass  # A/B logging is non-critical

        else:
            # ── Fallback: V3 → V1 single-model chain ─────────────────────────
    ...
    if ensemble_meta.get("model_votes"):
        votes_summary = ", ".join(f"{m}: {v.split(' | ')[-1]}" for m, v in ensemble_meta["model_votes"].items())
        decision_factors.append(f"Ensemble votes — {votes_summary}")

    reasoning = f"Categorized as '{classification['category']}' - {classification['subcategory']}."
    if ensemble_meta.get("ensemble_routing_action") == "escalate":
        reasoning += " ⚠ Extreme model disagreement detected — escalated to supervising admin."
    elif ensemble_meta.get("ensemble_routing_action") == "human_review":
        reasoning += " Routed to Human Review Queue due to low ensemble confidence."
    elif ensemble_meta.get("ensemble_routing_action") == "monitor":
        reasoning += " Auto-routed and flagged for monitoring (medium confidence)."
    ...
    # Determine needs_review from ensemble routing or confidence threshold
    routing_action = ensemble_meta.get("ensemble_routing_action", "")
    needs_review = (
        routing_action in ("human_review", "escalate") or
    ...
    # Merge ensemble metadata into env_metadata for frontend visibility
    if ensemble_meta:
        env_metadata["ensemble"] = {
            "entropy": ensemble_meta.get("ensemble_entropy"),
            "agreement": ensemble_meta.get("ensemble_agreement"),
            "routing_action": ensemble_meta.get("ensemble_routing_action"),
            "model_votes": ensemble_meta.get("model_votes"),
            "individual_confidences": ensemble_meta.get("individual_confidences"),
        }
...
async def analyze_stream(request_body: TicketRequest):
    ...
    async def event_generator():
        ...
        yield f"data: {json.dumps({'step': 'Running ensemble classification', 'status': 'in_progress'})}\n\n"
        await asyncio.sleep(0.2)
        stream_ensemble_meta = {}
        try:
            if _ENSEMBLE_AVAILABLE and ensemble_classifier:
                ensemble_result = ensemble_classifier.predict_with_metadata(text)
                cat = ensemble_result["category"]
                ...
                if "error" in classification_v3_res:
                    ...
                    classification = {
                        ...
                        "auto_resolve": auto_resolve, "assigned_team": assigned_team, "confidence": float(conf),
                    }
        except Exception as e:
            classification = {
                "category": "Unknown", "subcategory": "Unknown", "priority": "Medium",
...
# ---------------------------------------------------------------------------
# Ensemble Classification Endpoints (Issue #2805)
# ---------------------------------------------------------------------------
...
@app.post("/ai/ensemble/classify")
async def ensemble_classify(request: EnsembleRequest):
    """
    Run the full multi-model ensemble classification on a ticket text.
    Returns prediction, confidence, entropy, agreement score,
    routing action, per-model votes, and matched rules.
    """
    if not _ENSEMBLE_AVAILABLE or ensemble_classifier is None:
        raise HTTPException(
            status_code=503,
            detail="Ensemble classifier is not available. Check server logs."
        )
    try:
        result = ensemble_classifier.predict_with_metadata(request.text)
        if model_monitor:
...
@app.get("/ai/ensemble/health")
async def ensemble_health():
    """Check which ensemble models are loaded and operational."""
    status = {
        "ensemble_available": _ENSEMBLE_AVAILABLE,
        "models": {
            "bert_v3": classifier_v3.model is not None if hasattr(classifier_v3, 'model') else False,
            "bert_v1": classifier_service._loaded,
            "tfidf": False,
            "rf": False,
            "rules": False,
        }
    ...
    if _ENSEMBLE_AVAILABLE and ensemble_classifier:
        try:
            ...
            status["models"]["tfidf"] = tfidf_classifier._loaded
            status["models"]["rf"] = rf_classifier._loaded
            status["models"]["rules"] = True
            status["ensemble_weights"] = ensemble_classifier.weights
        except Exception:
            pass
    return status


@app.post("/ai/ensemble/weights")
async def update_ensemble_weights(request: EnsembleWeightsRequest):
    """
    Dynamically update ensemble model weights.
    Weights are normalized to sum to 1.
    Use for A/B testing or adapting to model drift.
    """
    if not _ENSEMBLE_AVAILABLE or ensemble_classifier is None:
        raise HTTPException(status_code=503, detail="Ensemble not available")
    try:
        new_weights = {
            "bert": request.bert,
            "tfidf": request.tfidf,
        ...
        ensemble_classifier.update_weights(new_weights)
        return {"status": "updated", "weights": ensemble_classifier.weights}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))


@app.get("/ai/ensemble/metrics")
async def ensemble_metrics():
    """
    Dashboard endpoint: returns ensemble monitoring metrics.
    Includes confidence distribution, escalation rate, drift indicators,
    category distribution, and A/B test summary.
    """
    if not model_monitor:
        raise HTTPException(status_code=503, detail="Model monitoring not available")
    metrics = model_monitor.get_dashboard_metrics()
    metrics["ab_test_summary"] = model_monitor.get_ab_test_summary()
    metrics["recent_drift_events"] = model_monitor.get_recent_drift_events(limit=10)
    return metrics


@app.post("/ai/ensemble/correct")
async def ensemble_log_correction(raw_request: Request):
    """
    Log an ensemble prediction correction for drift detection and A/B tracking.
    Body: { ticket_id, original_prediction, corrected_prediction, confidence }
    """
    if not model_monitor:
        raise HTTPException(status_code=503, detail="Model monitoring not available")
    try:
        body = await raw_request.json()
        model_monitor.log_correction(
            ticket_id=str(body.get("ticket_id", "unknown")),
            original_prediction=str(body.get("original_prediction", "")),
            corrected_prediction=str(body.get("corrected_prediction", "")),
