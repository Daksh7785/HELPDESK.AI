...
try:
    ...
    logger.error(f"[WARNING] Supabase initialization failed: {e}")
    supabase = None
    Client = None
...
async def lifespan(app: FastAPI):
    ...
    if gemini_service:
        logger.error(f"[Startup] Gemini Service: {'Initialized' if gemini_service._initialized else 'FAILED (Key missing or SDK error)'}")
    else:
        logger.error("[Startup] Gemini Service: NOT LOADED (Import failed)")
    ...
    try:
        ...
        if duplicate_text:
            try:
                ...
                logger.error(f"[WARNING] {duplicate_index_warning} ticket_id={ticket_id} error={index_error}")
        else:
            duplicate_indexed = False
